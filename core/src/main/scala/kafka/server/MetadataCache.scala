...
class ZkMetadataCache(brokerId: Int) extends MetadataCache with Logging {
  ...
  def updateMetadata(correlationId: Int, updateMetadataRequest: UpdateMetadataRequest): Seq[TopicPartition] = {
    inWriteLock(partitionMetadataLock) {
      ...
      // 从UpdateMetadataRequest中获取Controller所在的Broker ID // 如果当前没有Controller，赋值为None
      ...
      // 如果updateMetadataRequest 没有携带分区信息 直接使用 之前的分区信息和新的 Broker 列表信息 返回
      if (!updateMetadataRequest.partitionStates.iterator.hasNext) {
        ...
        // 使用更新过的分区元数据，和前面的存活Broker列表及节点列表，构建最新的元数据缓存
  ...
  // partitionStates: key 是主题名称，value: UpdateMetadataPartitionState
